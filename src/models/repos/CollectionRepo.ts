...
export type CollectionAnnotatedField = "vocabsByLevel" | "isBookmarked";

export class CollectionRepo extends EntityRepository<Collection> {

    async annotateCollectionsWithUserData(collections: Collection[], user: User, fields: CollectionAnnotatedField[] = ["vocabsByLevel", "isBookmarked"]) {
    ...
    async annotateVocabsByLevel(collections: Collection[], learnerId: number) {
    ...
    async annotateIsBookmarked(collections: Collection[], learnerId: number) {
        if (collections.length === 0)
            return collections;
        const query = `SELECT json_object_agg(collection_id, true) AS collection_id_to_is_bookmarked
                       FROM collection_bookmark
                       WHERE bookmarker_id = ${learnerId}
                         AND collection_id IN (${collections.map(c => c.id).join(",")});`;
